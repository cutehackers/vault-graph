...
- Episodic 또는 timeline memory는 durable Vault-derived document change와 derived
  projection change 위에 만든 Phase 6C timeline projection입니다. Hidden transcript
  또는 raw session log store가 아닙니다.
...
Phase 6C timeline output은 durable Vault-derived document change와 derived
projection change를 결합합니다. 각 timeline item은 origin을 표시해야 합니다.
