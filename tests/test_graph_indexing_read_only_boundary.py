...
def test_graph_apply_and_status_do_not_modify_vault_or_projection_cache(
    ...
    before_status_state = _tree_snapshot(graph_home_path)
    status_result = runner.invoke(app, ["status", "--graph-home", str(graph_home_path)])

    assert index_result.exit_code == 0
    assert status_result.exit_code == 0
    assert _tree_snapshot(vault_root) == before_vault
    assert _tree_snapshot(graph_home_path) == before_status_state
    assert not (graph_home_path / "projection_cache").exists()
