...
def test_current_context_resource_returns_single_vault_project_memory_projection(tmp_path: Path) -> None:
    ...
    seed_project_status(graph_home_path, vault_id="default")
...
def test_current_context_resource_does_not_return_all_vault_summary(tmp_path: Path) -> None:
    ...
    CatalogService(graph_home_path=graph_home_path).save_catalog(
    ...
    seed_project_status(graph_home_path, vault_id="main")
    seed_project_status(graph_home_path, vault_id="work")
    registered = create_mcp_server(McpServerConfig(graph_home_path=graph_home_path))
...
def test_current_context_resource_preserves_memory_warnings_in_body_and_metadata(tmp_path: Path) -> None:
    ...
    vault_root.mkdir()
    graph_home_path = initialized_state(tmp_path, vault_root)
    SQLiteMetadataStore(graph_home_path / "metadata" / "metadata.sqlite3", initialize=True)
    registered = create_mcp_server(McpServerConfig(graph_home_path=graph_home_path))
...
def seed_project_status(graph_home_path: Path, *, vault_id: str) -> None:
